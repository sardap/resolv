...
type Shape interface {
	...
	WouldBeColliding(Shape, float32, float32) bool
	GetTags() []string
	ClearTags()
	AddTags(...string)
	RemoveTags(...string)
...
func (b *BasicShape) GetXY() (float32, float32) {
	return b.X, b.Y
}

// SetXY sets the position of the Shape.
func (b *BasicShape) SetXY(x, y float32) {
	b.X = x
	b.Y = y
}

// Move moves the Shape by the delta X and Y values provided.
func (b *BasicShape) Move(x, y float32) {
	b.X += x
	b.Y += y
}
